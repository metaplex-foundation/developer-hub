...
function processExampleDirectory(dir) {
  ...
  for (const [key, fileInfo] of Object.entries(nativeFiles)) {
    if (fs.existsSync(fileInfo.path)) {
      ...
      fileMeta[key] = { isTs: !!fileInfo.isTs }
      hasNativeFiles = true
    }
  }
  ...
  if (existingFiles.kit) {
    const kitLang = fileMeta.kit && fileMeta.kit.isTs ? 'typescript' : 'javascript'
    lines.push('  kit: {')
  ...
  if (existingFiles.umi) {
    const umiLang = fileMeta.umi && fileMeta.umi.isTs ? 'typescript' : 'javascript'
    lines.push('  umi: {')
  ...
  if (existingFiles.das) {
    const dasLang = fileMeta.das && fileMeta.das.isTs ? 'typescript' : 'javascript'
    lines.push('  das: {')
  ...
  console.log(`✓ Generated ${path.relative(examplesDir, indexPath)}`)
}
