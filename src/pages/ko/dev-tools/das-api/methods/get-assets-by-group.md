...
그룹(키, 값) 쌍이 지정된 자산 목록을 반환합니다.

Token Metadata 및 mpl-core 컬렉션에는 `groupKey: "collection"`을 사용합니다. [mpl-core GroupV1](/ko/smart-contracts/core) 계정의 멤버(컬렉션, 자산, 중첩 그룹)를 가져오려면 `groupKey: "group"`을 사용합니다.

모든 자산을 나열하지 않고 그룹 이름과 멤버 수를 가져오려면 [`getGrouping`](/ko/dev-tools/das-api/methods/get-grouping)을 사용하세요.
