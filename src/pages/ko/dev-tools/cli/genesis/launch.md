...
2단계에서 "Agent is not owned by the connected wallet" 오류가 발생하면, API 백엔드가 아직 새 agent를 인덱싱하지 않은 것입니다. 온체인 토큰 생성은 이미 성공했을 수 있습니다 — `mplx agents fetch <ASSET>`로 확인하세요. Agent에 이미 토큰이 설정되어 있으면 플랫폼 등록만 실패한 것이므로 `mplx genesis launch register`로 완료하세요. 두 단계를 스크립팅할 때는 agent 등록과 출시 명령어 사이에 약 30초의 지연을 추가하세요.
{% /callout %}
