...
programmingLanguage:
  - Bash
faqs:
  - q: genesis launch create와 수동 플로우의 차이점은 무엇인가요?
    a: genesis launch create 명령어는 Genesis API를 호출하여 트랜잭션을 빌드하고, 서명 및 전송하며, Metaplex 플랫폼에 출시를 등록하는 올인원 플로우입니다 — 모두 단일 명령으로 실행됩니다. 수동 플로우는 별도의 create, bucket, finalize 및 register 단계가 필요합니다.
  - q: genesis launch register는 언제 사용해야 하나요?
    a: 저수준 CLI 명령어(genesis create, bucket add-launch-pool 등)를 사용하여 이미 genesis 계정을 생성했고 공개 출시 페이지를 위해 Metaplex 플랫폼에 등록하고 싶을 때 genesis launch register를 사용합니다.
...
Genesis API를 사용하여 단일 명령으로 토큰 출시를 생성하고 등록:
...
`genesis launch` 명령어는 Genesis API를 사용하여 간소화된 방식으로 토큰을 출시합니다. genesis 계정을 수동으로 생성하고, bucket을 추가하고, finalize하고, 별도로 등록하는 대신 API가 전체 플로우를 처리합니다.
...
- **metaplex.com 호환**: API를 통해 생성하거나 등록한 출시는 [metaplex.com](https://metaplex.com)에 공개 출시 페이지와 함께 표시
...
**바로 이동:** [Launch Create](#launch-create) · [Launch Register](#launch-register) · [잠금 할당](#locked-allocations) · [일반적인 오류](#common-errors) · [FAQ](#faq)

## Launch Create

`mplx genesis launch create` 명령어는 Genesis API를 통해 새로운 토큰 출시를 생성합니다. 전체 플로우를 처리합니다:
...
| 플래그 | 설명 |
|--------|------|
| `--name <string>` | 토큰 이름 (1–32자) |
| `--symbol <string>` | 토큰 심볼 (1–10자) |
| `--image <string>` | 토큰 이미지 URL (현재 `https://gateway.irys.xyz/`로 시작해야 함) |
| `--tokenAllocation <integer>` | Launch Pool 토큰 할당량 (10억 총 공급량의 일부) |
| `--depositStartTime <string>` | 예치 시작 시간 (ISO 날짜 문자열 또는 Unix 타임스탬프) |
| `--raiseGoal <integer>` | 모금 목표 (정수 단위, 예: 200 SOL의 경우 200) |
| `--raydiumLiquidityBps <integer>` | Raydium 유동성 (basis point 단위, 2000–10000, 즉 20%–100%) |
| `--fundsRecipient <string>` | 자금 수령 지갑 주소 |

### 선택적 플래그

| 플래그 | 설명 | 기본값 |
|--------|------|--------|
| `--description <string>` | 토큰 설명 (최대 250자) | — |
...
| `--quoteMint <string>` | Quote 민트 (현재 `SOL` 또는 `USDC` 지원) | `SOL` |
...
3. 메타데이터 및 잠금 할당 포함:
```bash {% title="메타데이터와 할당이 포함된 전체 출시" %}
mplx genesis launch create \
  --name "My Token" \
  --symbol "MTK" \
  --image "https://gateway.irys.xyz/abc123" \
...
`mplx genesis launch register` 명령어는 기존 genesis 계정을 Metaplex 플랫폼에 등록합니다. 저수준 CLI 명령어(`genesis create`, `bucket add-launch-pool` 등)로 genesis 계정을 생성하고 공개 출시 페이지를 원할 때 사용합니다.
...
Launch Config JSON 파일은 `launch create` 입력과 동일한 형식을 사용합니다:
...
계정이 이미 등록된 경우, 명령어는 이를 알리고 기존 출시 세부 정보를 표시합니다.
...
| `tokenAmount` | number | 할당할 토큰 수량 |
| `vestingStartTime` | string | 베스팅 시작 ISO 날짜 문자열 |
| `vestingDuration` | object | `value` (숫자)와 `unit`을 가진 기간 |
...
| 오류 | 원인 | 해결 방법 |
|------|------|-----------|
| API request failed | 네트워크 문제 또는 잘못된 입력 | 오류 응답 세부 정보를 확인하세요 — 명령어는 유효성 검사 오류 시 API 응답 본문을 표시합니다 |
| Locked allocations file not found | 잘못된 파일 경로 | 할당 JSON 파일의 경로를 확인하세요 |
...
| Launch config missing required fields | register용 구성이 불완전 | Launch Config JSON에 `token`, `launch` 및 `launchType: "project"`가 있는지 확인하세요 |

## FAQ

**`genesis launch create`와 수동 플로우의 차이점은 무엇인가요?**
`genesis launch create` 명령어는 Genesis API를 호출하여 트랜잭션을 빌드하고, 서명 및 전송하며, Metaplex 플랫폼에 출시를 등록하는 올인원 플로우입니다 — 모두 단일 명령으로 실행됩니다. 수동 플로우는 별도의 `create`, `bucket add-launch-pool`, `finalize` 및 register 단계가 필요합니다.

**`genesis launch register`는 언제 사용해야 하나요?**
저수준 CLI 명령어(`genesis create`, `bucket add-launch-pool` 등)를 사용하여 이미 genesis 계정을 생성했고 공개 출시 페이지를 위해 Metaplex 플랫폼에 등록하고 싶을 때 `genesis launch register`를 사용합니다.

**launch 명령어는 어떤 네트워크를 사용하나요?**
네트워크는 구성된 RPC 엔드포인트에서 자동 감지됩니다. `--network` 플래그(`solana-mainnet` 또는 `solana-devnet`)로 재정의할 수 있습니다.

**사용자 정의 Quote 민트를 사용할 수 있나요?**
API는 현재 `SOL`(기본값)과 `USDC`를 지원합니다. USDC를 사용하려면 `--quoteMint USDC`를 전달하세요.

**총 토큰 공급량은 얼마인가요?**
API 플로우를 사용할 때 총 공급량은 현재 1,000,000,000 토큰으로 고정되어 있습니다.
