...
{% quick-link title="에이전트 등록" icon="InboxArrowDown" href="/agents/register-agent" description="Core 에셋을 민팅하고 Metaplex 에이전트 레지스트리에 등록하는 단계별 가이드." /%}

{% quick-link title="Metaplex Skill" icon="CodeBracketSquare" href="/agents/skill" description="코딩 에이전트에게 Metaplex 프로그램에 대한 최신 지식을 제공합니다." /%}
...
**지갑 생성 및 입금** — 메인 지갑을 생성하고 [Core](/core) 에셋 등록 및 지속적인 트랜잭션 수수료를 충당하기 위해 최소 0.2 SOL을 입금합니다.

**에이전트 등록** — EIP-8004 에이전트 표준을 준수하는 메타데이터를 가진 에이전트의 온체인 신원 역할을 하는 [Core](/core) 에셋을 민팅합니다. 이를 통해 에이전트의 Core 에셋 주소가 생성되며, 이는 모든 후속 작업에 필요합니다.
...
**토큰 출시 (선택 사항)** — LaunchPool(48시간 예치 기간, 최소 250 SOL 또는 25,000 USDC 모금) 또는 Bonding Curve(즉시 거래, 최소 없음)를 사용하여 [Genesis](/smart-contracts/genesis)로 토큰을 생성합니다.
...
- 등록으로 생성된 Core 에셋 주소는 [에이전트 토큰 생성](/agents/create-agent-token), [에이전트 커머스](/agents/agentic-commerce) 및 기타 에이전트 워크플로에 필요합니다
