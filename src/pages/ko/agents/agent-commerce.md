...
faqs:
  ...
  - q: Metaplex는 x402 결제를 지원하나요?
    a: 네, 양쪽 모두 지원합니다. 에이전트 메타데이터에는 거래 상대방이 에이전트가 HTTP 402 스테이블코인 결제용으로 설정되어 있는지 발견할 수 있도록 1급 시민 `x402Support` 부울 플래그가 포함되어 있고, 에이전트의 PDA 지갑은 USDC를 포함한 모든 SPL 토큰을 보유하며, 그 이그제큐티브는 외부 결제에 서명합니다. 또한 Metaplex는 에이전트와 지갑에 LLM 추론, 이미지 생성, Solana RPC를 판매하는 라이브 요청당 과금 API인 Metaplex x402를 운영합니다.
...
Metaplex의 에이전트 커머스는 등록된 에이전트의 생산적 경제 활동입니다 — EIP-8004 메타데이터를 통해 서로를 발견하고, 서비스에 대해 비용을 청구하고, 스테이블코인으로 거래 상대방에게 비용을 지불하고, 이그제큐티브가 서명한 트랜잭션을 통해 온체인에서 결제합니다. [에이전트 파이낸스](/ko/agents/agent-finance)가 에이전트가 어떻게 자본화되는지를 다룬다면, 에이전트 커머스는 에이전트가 그 자본으로 무엇을 하는지, 그리고 어떻게 자기 몫을 벌어들이는지를 다룹니다. {% .lead %}
...
- **이그제큐티브 위임**: [`mpl-agent-tools`](/ko/smart-contracts/mpl-agent/tools)는 자산별 `ExecutionDelegateRecordV1` PDA를 발행하므로 이그제큐티브는 에이전트를 대신하여 결제에 서명할 수 있고 소유자는 언제든지 취소할 수 있습니다

{% callout type="note" title="에이전트 커머스 vs. 에이전트 파이낸스" %}
**에이전트 커머스**는 *생산적 활동*에 관한 것입니다 — 에이전트가 어떻게 거래 상대방을 발견하고, 수익을 얻고, 결제하고, 거래하는지. **[에이전트 파이낸스](/ko/agents/agent-finance)**는 *자본화 및 거버넌스*에 관한 것입니다 — 에이전트가 어떻게 자금을 조달받고 홀더가 그 미션과 어떻게 정렬되는지. 파이낸스는 에이전트를 부트스트랩하고; 커머스는 에이전트가 자립하는 방법입니다.
{% /callout %}
...
| 프리미티브 | 위치 | 가능한 기능 |
|-----------|----------------|-----------------|
| **온체인 신원** | MPL Core 자산에 바인딩된 [`AgentIdentityV2`](/ko/smart-contracts/mpl-agent/identity) PDA | 거래 상대방은 에이전트의 신원을 도메인이나 지갑이 아닌 온체인에서 검증 |
...
| **PDA 지갑(Asset Signer)** | [MPL Core](/ko/smart-contracts/core)에 의해 파생된 시드 `["mpl-core-execute", asset]` | 모든 SPL 토큰을 보유하고 사용; 개인 키 없음 |
| **이그제큐티브 위임** | `mpl-agent-tools`의 [`ExecutionDelegateRecordV1`](/ko/smart-contracts/mpl-agent/tools) PDA | 오프체인 운영자가 에이전트를 대신하여 서명; 자산별; 취소 가능 |
| **토큰 바인딩** | `AgentIdentityV2`의 [`setAgentTokenV1`](/ko/dev-tools/cli/agents/set-agent-token) | 수익 라우팅을 위한 에이전트와 그 [토큰](/ko/agents/agent-finance) 간의 영구 링크 |
...
## Metaplex의 x402 스테이블코인 결제

Metaplex는 [Metaplex x402](/ko/agents/x402)를 운영합니다. LLM 추론, 이미지 생성, Solana RPC를 USDC로 에이전트에게 판매하는 라이브 요청당 과금 API입니다. 이 서비스는 [x402](https://www.x402.org)를 구현합니다. x402는 HTTP `402 Payment Required`를 사용하여 스테이블코인 마이크로페이먼트를 API 액세스의 1급 시민으로 만드는 새로운 프로토콜입니다. 클라이언트는 리소스를 요청하고, 결제 지시와 함께 `402`를 받고, 온체인에서 결제하고, 결제 증명과 함께 재시도합니다.

Metaplex는 양쪽 모두를 제공합니다. *에이전트* 측에서는 등록이 프로토콜에 필요한 모든 것을 담고 있습니다:
...
*서비스* 측에서는 [Metaplex x402](/ko/agents/x402)가 LLM 추론, 이미지 생성, Solana RPC를 판매하는 라이브 요청당 과금 API이며, 오픈소스 클라이언트는 [`@metaplex-foundation/x402`](https://github.com/metaplex-foundation/x402)에 있습니다. 등록된 에이전트는 [결제 권한을 한 번 위임](/ko/agents/x402/payment-modes#pay-instantly-with-a-delegated-agent)하면 이후 추가 서명 없이 자신의 지갑에서 요청마다 결제할 수 있습니다.
...
3. **결제** — 유료 서비스의 경우, 서버는 HTTP 402(또는 유사한 게이팅)를 반환합니다; 클라이언트는 USDC 또는 다른 스테이블코인으로 에이전트의 [Asset Signer PDA](/ko/agents/what-is-an-agent)에 결제합니다
4. **검증** — 서버는 온체인 결제를 읽고, 발신자(선택적으로 신뢰 점수를 위한 발신자 자체의 에이전트 등록)를 확인하고, 리소스의 잠금을 해제합니다
5. **외부 결제** — 에이전트 자체가 거래 상대방(컴퓨팅, 데이터, 다른 에이전트)에게 비용을 지불해야 할 때, 그 [이그제큐티브](/ko/agents/run-an-agent)는 Core Execute 인스트럭션으로 래핑된 외부 송금에 서명합니다

각 단계는 Metaplex 스택이 이미 제공하는 프리미티브를 사용합니다. 오프체인 보관자나 플랫폼 중개 에스크로가 없습니다.

## 참고 사항

- 이 페이지는 에이전트 측 빌딩 블록을 설명합니다. Metaplex가 운영하는 x402 서비스 자체는 [Metaplex x402](/ko/agents/x402)에서 다루며, 인덱스화된 에이전트 디렉토리는 별도의 런타임 문제로 자체 가이드를 받을 것입니다
- EIP-8004는 메타데이터 형식입니다; [에이전트 파이낸스](/ko/agents/agent-finance)와 에이전트 커머스는 그 위의 레이어입니다. 같은 등록 문서가 둘 다에 의해 읽힙니다
- `AgentIdentityV2`의 `agentToken` 필드는 [`setAgentTokenV1`](/ko/dev-tools/cli/agents/set-agent-token)을 통해 한 번 설정되며 영구적입니다. 에이전트의 토큰 홀더에 대한 수익 라우팅은 파이낸스 문제입니다; 커머스 흐름은 SOL이나 스테이블코인을 에이전트의 PDA로 직접 라우팅할 수 있습니다
...
[에이전트 파이낸스](/ko/agents/agent-finance)는 에이전트가 토큰을 통해 어떻게 **자본화되고 거버넌스되는지**를 다룹니다. 에이전트 커머스는 에이전트가 그 후 어떻게 **수익을 얻고, 지출하고, 거래하는지**를 다룹니다. 파이낸스는 에이전트에 자금을 제공하고; 커머스는 에이전트가 그 자금으로 무엇을 하는지입니다.
...
네, 양쪽 모두 지원합니다. 에이전트 메타데이터에는 능력 발견을 위한 1급 시민 `x402Support` 부울이 있고, PDA 지갑은 USDC를 보유하며, 이그제큐티브는 외부 결제에 서명합니다. 또한 Metaplex는 추론, 이미지 생성, Solana RPC를 위한 라이브 요청당 과금 API인 [Metaplex x402](/ko/agents/x402)를 운영합니다.
...
예. 에이전트의 PDA 지갑(Asset Signer, `["mpl-core-execute", asset]`로 파생됨)은 모든 SPL 토큰을 받아들입니다. 개인 키가 없습니다 — 지갑은 Core의 Execute 라이프사이클 훅을 통해서만 제어되며, [`mpl-agent-tools`](/ko/smart-contracts/mpl-agent/tools)를 통해 자산의 이그제큐티브에 의해 서명됩니다.

### 에이전트는 자율적으로 서비스에 대해 어떻게 비용을 지불하나요?
이그제큐티브는 Core의 [Execute 라이프사이클 훅](/ko/smart-contracts/core/execute-asset-signing)을 통해 외부 트랜잭션에 서명합니다. 결제 게이트 API의 경우, 에이전트의 결제 클라이언트(x402 또는 기타)가 송금을 구성하고, 이그제큐티브가 서명하고, API 서버는 리소스 잠금을 해제하기 전에 온체인 결제를 검증합니다.
...
| **Agent Finance(에이전트 파이낸스)** | 자체 토큰을 통해 에이전트를 자본화하고 거버넌스하는 관행([에이전트 파이낸스](/ko/agents/agent-finance) 페이지에서 다룸) |
...
| **Executive Profile(이그제큐티브 프로필)** | [`mpl-agent-tools`](/ko/smart-contracts/mpl-agent/tools)를 통해 등록된, 에이전트를 대신하여 서명할 권한을 가진 오프체인 운영자의 온체인 신원 |
