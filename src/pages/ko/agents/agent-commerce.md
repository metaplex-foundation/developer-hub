...
*서비스* 측에서는 [Metaplex x402](/agents/x402)가 LLM 추론, 이미지 생성, Solana RPC를 판매하는 라이브 요청당 과금 API이며, 오픈소스 클라이언트는 [`@metaplex-foundation/x402`](https://github.com/metaplex-foundation/x402)에 있습니다. 등록된 에이전트는 [결제 권한을 한 번 위임](/agents/x402/payment-modes#pay-instantly-with-a-delegated-agent)하면 이후 추가 서명 없이 자신의 지갑에서 요청마다 결제할 수 있습니다.
...
- 이 페이지는 에이전트 측 빌딩 블록을 설명합니다. Metaplex가 운영하는 x402 서비스 자체는 [Metaplex x402](/agents/x402)에서 다루며, 인덱스화된 에이전트 디렉토리는 별도의 런타임 문제로 자체 가이드를 받을 것입니다
...
네, 양쪽 모두 지원합니다. 에이전트 메타데이터에는 능력 발견을 위한 1급 시민 `x402Support` 부울이 있고, PDA 지갑은 USDC를 보유하며, 이그제큐티브는 외부 결제에 서명합니다. 또한 Metaplex는 추론, 이미지 생성, Solana RPC를 위한 라이브 요청당 과금 API인 [Metaplex x402](/agents/x402)를 운영합니다.
