...
**CLI** (`mplx agents`): 에이전트 아이덴티티 등록, 실행 위임 및 취소, 에이전트 데이터 조회, Genesis 토큰을 에이전트에 연결. 전체 에이전트 토큰 생성 플로우에는 `mplx genesis launch create --agentAsset --agentSetToken`을 사용하여 런치와 연결을 한 번에 실행.
...
| 에이전트 토큰 출시 (Genesis + 연결) | Yes (`mplx genesis launch create --agentAsset --agentSetToken`) |
