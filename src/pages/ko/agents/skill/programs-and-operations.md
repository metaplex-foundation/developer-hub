...
공정한 배포와 Raydium으로의 자동 유동성 졸업을 갖춘 토큰 출시 프로토콜. 두 가지 출시 유형 지원: **런치풀** (48시간 예치 기간과 비례 배분) 및 **본딩 커브** (즉시 거래와 Raydium CPMM 자동 졸업).

**CLI** (`mplx genesis`): 토큰 출시 생성 및 관리. `genesis launch create` 명령은 런치풀과 본딩 커브 모두에 대한 올인원 API 플로우를 제공합니다 — `--agentMint`를 통한 [에이전트 통합](/agents/mint-agent)도 선택적으로 사용 가능.

**Umi SDK**: 본딩 커브 설정과 에이전트 토큰 연결을 포함한 토큰 출시 생성 및 관리를 위한 완전한 프로그래밍 접근.
...
| 토큰 출시 (Genesis) | Yes (런치풀 및 본딩 커브) |
| 에이전트 토큰 출시 | Yes (`--agentMint` 플래그) |
| Core NFT 소각 | Yes |
