...
자산 계정의 중요한 속성 중 하나는 오프체인의 JSON 파일을 가리키는 `URI` 속성입니다. 이는 온체인 데이터 저장과 관련된 수수료의 제약을 받지 않으면서 안전하게 추가 데이터를 제공하는 데 사용됩니다. 그 JSON 파일은 누구나 토큰에서 유용한 정보를 찾는 데 사용할 수 있는 [특정 표준](/ko/token-metadata/token-standard)을 따릅니다.
