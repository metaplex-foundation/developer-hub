...
description: 캔디 가드는 Core Candy Machine에 부착되어 민팅 프로세스를 제한하고 커스터마이즈하는 모듈형 액세스 제어 컴포넌트입니다. 가드 유형, Candy Guard 계정, 사용 가능한 가드, 그리고 이를 조합하는 방법에 대해 알아보세요.
keywords:
  - candy guard
  - guard
  - access control
...
updated: '03-10-2026'
faqs:
  - q: 커스텀 캔디 가드를 만들 수 있나요?
    a: 네. 가드는 별도의 Candy Guard 프로그램에 존재하므로 누구나 자신만의 Candy Guard 프로그램을 포크하고 배포하여 커스텀 가드 로직을 만들 수 있으며, 민팅은 Core Candy Machine Core 프로그램에 의존합니다. Metaplex SDK를 사용하면 커스텀 Candy Guard 프로그램을 등록하여 표준 API를 사용할 수도 있습니다.
  - q: 하나의 Core Candy Machine에서 얼마나 많은 가드를 동시에 사용할 수 있나요?
    a: 사용 가능한 가드의 어떤 조합이든 동시에 활성화할 수 있습니다. 가드는 조합 가능하므로 필요한 것만 활성화하면 됩니다. 더 복잡한 시나리오에서는 가드 그룹을 사용하여 단일 머신에 여러 가드 세트를 정의할 수도 있습니다.
  - q: 모든 가드에 민트 설정이나 라우트 지시사항이 필요한가요?
    a: 아니요. 특정 가드만 추가 온체인 계정(민트 설정)이나 전용 라우트 지시사항이 필요합니다. 대부분의 가드는 자체적으로 완결됩니다. 민트 설정이나 라우트 지시사항이 적용되는지 확인하려면 개별 가드 페이지를 확인하세요.
  - q: 민터가 가드 검사에 실패하면 어떻게 되나요?
    a: 트랜잭션이 거부됩니다. Bot Tax 가드가 활성화되어 있으면 실패한 지갑에 구성 가능한 SOL 패널티가 부과되어 봇의 잘못된 민트 시도 스팸을 방지합니다.
  - q: Core Candy Machine의 가드를 업데이트하면 기존 가드 설정이 모두 교체되나요?
    a: 네. 가드 업데이트는 전체 가드 구성을 덮어씁니다. 변경하는 가드뿐만 아니라 활성화하려는 모든 가드를 다시 지정해야 합니다.
  - q: Candy Guard와 가드 그룹의 차이점은 무엇인가요?
    a: Candy Guard는 하나의 기본 가드 세트를 보유하는 온체인 계정입니다. 가드 그룹을 사용하면 동일한 Candy Guard 계정 내에서 여러 개의 명명된 가드 세트를 정의하여 서로 다른 지갑이나 단계가 서로 다른 규칙을 따를 수 있습니다.
---

## 요약

캔디 가드는 [Core Candy Machine](/ko/smart-contracts/core-candy-machine)에 부착되어 Solana에서의 [민팅](/ko/smart-contracts/core-candy-machine/mint) 프로세스를 제한하고 커스터마이즈하는 모듈형, 조합 가능한 액세스 제어 컴포넌트입니다. {% .lead %}

- 각 가드는 결제, 스케줄링, 지갑 게이팅 등 단일 책임을 처리합니다. {% .lead %}
- 가드는 Candy Machine의 민트 권한이 되는 별도의 온체인 Core Candy Guard 계정에 정의됩니다. {% .lead %}
- 기본 Candy Guard 프로그램에는 SOL/토큰 결제, 허용 목록, 시간 윈도우, 봇 보호 등을 포함하는 25개 이상의 내장 가드가 포함되어 있습니다. {% .lead %}
- 커스텀 가드는 자신만의 Candy Guard 프로그램을 포크하고 배포하여 만들 수 있습니다. {% .lead %}

## 캔디 가드란 무엇인가?

캔디 가드는 [Core Candy Machine](/ko/smart-contracts/core-candy-machine)의 [민팅](/ko/smart-contracts/core-candy-machine/mint) 프로세스 중에 단일 액세스 제어 규칙을 적용하는 모듈형 온체인 컴포넌트입니다. 각 가드는 독립적으로 활성화되며 다른 가드와 결합하여 필요한 민팅 경험을 정확하게 구성할 수 있습니다.

선택할 수 있는 가드 세트가 광범위하며, 각 가드는 필요에 따라 활성화하고 구성할 수 있습니다.

이 문서의 뒷부분에서 [사용 가능한 모든 가드](#사용-가능한-가드)에 대해 다루겠지만, 먼저 몇 가지 예시를 통해 이를 설명해보겠습니다.
...
코어 캔디 가드 계정은 주어진 [Core Candy Machine](/ko/smart-contracts/core-candy-machine)에 대해 활성화된 모든 가드와 그 구성을 저장하는 온체인 계정입니다. 각 코어 캔디 머신 계정은 일반적으로 자체 코어 캔디 가드 계정과 연결되어 보호 계층을 추가합니다.
...
- 커스텀 가드를 만들려면 자신만의 Candy Guard 프로그램을 포크하고 배포해야 합니다. [Core](/ko/smart-contracts/core) Candy Machine Core 프로그램 자체는 변경할 필요가 없습니다.
- Core Candy Machine의 가드를 업데이트하면 전체 가드 구성이 덮어쓰여집니다. 수정하는 가드뿐만 아니라 활성화하려는 모든 가드를 항상 다시 지정하세요.
- [Allow List](/ko/smart-contracts/core-candy-machine/guards/allow-list)와 같은 일부 가드는 전제 조건을 검증하기 위해 민팅 전에 [라우트 지시사항](/ko/smart-contracts/core-candy-machine/guard-route)을 호출해야 합니다.
- [가드 그룹](/ko/smart-contracts/core-candy-machine/guard-groups)을 사용하여 가드를 여러 개의 명명된 세트로 구성할 수 있으며, 이를 통해 서로 다른 민팅 단계나 지갑 등급에 서로 다른 규칙을 적용할 수 있습니다.
...
사용 가능한 가드의 어떤 조합이든 동시에 활성화할 수 있습니다. 가드는 조합 가능하므로 필요한 것만 활성화하면 됩니다. 더 복잡한 시나리오에서는 [가드 그룹](/ko/smart-contracts/core-candy-machine/guard-groups)을 사용하여 단일 머신에 여러 가드 세트를 정의할 수도 있습니다.

### 모든 가드에 민트 설정이나 라우트 지시사항이 필요한가요?

아니요. 특정 가드만 추가 온체인 계정(민트 설정)이나 전용 [라우트 지시사항](/ko/smart-contracts/core-candy-machine/guard-route)이 필요합니다. 대부분의 가드는 자체적으로 완결됩니다. 민트 설정이나 라우트 지시사항이 적용되는지 확인하려면 개별 가드 페이지를 확인하세요.

### 민터가 가드 검사에 실패하면 어떻게 되나요?

트랜잭션이 거부됩니다. [Bot Tax](/ko/smart-contracts/core-candy-machine/guards/bot-tax) 가드가 활성화되어 있으면 실패한 지갑에 구성 가능한 SOL 패널티가 부과되며, 이는 봇의 잘못된 민트 시도 스팸을 방지합니다.
...
Candy Guard는 하나의 기본 가드 세트를 보유하는 온체인 계정입니다. [가드 그룹](/ko/smart-contracts/core-candy-machine/guard-groups)을 사용하면 동일한 Candy Guard 계정 내에서 여러 개의 명명된 가드 세트를 정의하여 서로 다른 지갑이나 단계가 서로 다른 규칙을 따를 수 있습니다.
...
| Candy Guard | Core Candy Machine에 대해 활성화된 전체 가드 세트를 저장하는 온체인 계정입니다. |
| Candy Guard Program | Candy Guard 계정을 소유하고 Core Candy Machine Core 프로그램에 위임하기 전에 모든 가드 조건을 평가하는 Solana 프로그램입니다. |
| Mint Authority (민트 권한) | Core Candy Machine에서 민트를 호출할 수 있는 공개 키입니다. 가드가 활성화되면 Candy Guard 계정으로 설정됩니다. |
| Sol Payment | 민팅 지갑이 대상 지갑에 지정된 SOL 금액을 지불하도록 요구하는 가드입니다. |
| Bot Tax | 실패한 민트 트랜잭션에 구성 가능한 SOL 패널티를 부과하여 봇을 방지하는 가드입니다. |
...
| Route Instruction (라우트 지시사항) | 일부 가드가 전제 조건을 검증하거나 설정하기 위해 민팅 전에 호출해야 하는 전용 지시사항입니다. [가드 라우트](/ko/smart-contracts/core-candy-machine/guard-route)를 참조하세요. |
