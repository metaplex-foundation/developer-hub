...
MPL-404 에스크로 및 스왑 프로그램을 배포하기 전에 사용할 NFT 컬렉션과 대체 가능한 토큰을 준비해야 합니다. 아직 준비되지 않았다면, 이 페이지의 나머지 부분을 읽은 후에 대체 불가능한 토큰에는 [Metaplex Core](/ko/smart-contracts/core)를, 대체 가능한 토큰을 생성하기 위해서는 [Token Metadata program](/ko/smart-contracts/token-metadata)을 사용하는 것을 권장합니다.
