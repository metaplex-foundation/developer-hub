...
아래 인터랙티브 계산기를 사용하여 트리 생성 비용을 추정할 수 있습니다. 필요한 cNFT 수를 입력하거나 고급 모드를 사용하여 모든 트리 매개변수를 조정하고 비용, 증명 크기 및 트리 구조에 미치는 영향을 확인하세요.
...
- **공개**: Bubblegum 트리를 공개할지 여부입니다. 공개인 경우 누구나 압축 NFT를 발행할 수 있습니다. 그렇지 않으면 트리 생성자 또는 트리 위임자([cNFT 위임](/ko/smart-contracts/bubblegum/delegate-cnfts)에서 논의됨)만 압축 NFT를 발행할 수 있습니다.
...
- **변경 로그**(또는 루트), **시퀀스 번호** 등과 같은 트리에 대한 저수준 정보를 저장하는 **트리** 자체. 이 문서의 [전용 페이지](/ko/smart-contracts/bubblegum-v2/concurrent-merkle-trees)에서 동시 Merkle 트리에 대해 더 자세히 다룹니다.
- [Merkle 트리 캐노피](/ko/smart-contracts/bubblegum-v2/merkle-tree-canopy) 페이지에서 논의된 **캐노피**.
