...
컬렉션과 그룹은 서로 다른 문제를 해결합니다. 컬렉션은 멤버 NFT를 관리하고, 그룹은 컬렉션·에셋·자식 그룹의 상위 분류를 제공합니다.
...
| **Groups 플러그인** | 멤버에 부여되어 부모 그룹 공개 키를 저장하는 권한 관리 플러그인 |
| **직접 멤버** | 그룹의 온체인 벡터에 명시적으로 나열된 컬렉션, 에셋 또는 자식 그룹 |

## 빠른 참조

아래 표는 일반적인 그룹 작업의 mpl-core 프로그램 ID와 SDK 헬퍼를 나열합니다.

### 프로그램 ID

mpl-core 프로그램 ID는 mainnet과 devnet에서 동일합니다.
...
생성, 조회, 업데이트, 멤버십 작업에는 다음 SDK 함수를 사용합니다.
...
컬렉션은 공유 메타데이터와 컬렉션 레벨 플러그인 아래에서 Core Asset을 그룹화합니다. 그룹은 컬렉션, 스탠드얼론 에셋, 다른 그룹을 참조할 수 있는 분류 컨테이너입니다. 컬렉션은 "이 NFT가 어떤 시리즈에 속하는가?"에 답하고, 그룹은 "이 컬렉션이나 에셋이 어떤 상위 세트에 포함되는가?"에 답합니다.

### 컬렉션이 여러 그룹에 속할 수 있나요?

네. 컬렉션을 그룹에 추가하면 mpl-core가 부모 그룹 주소를 컬렉션의 Groups 플러그인에 기록합니다. 컬렉션은 온체인 벡터 한도까지 여러 부모 그룹을 나열할 수 있습니다.

### 그룹을 중첩할 수 있나요?

네. 그룹은 자식 그룹을 포함하고 부모 그룹도 나열할 수 있습니다. 부모/자식 링크는 두 계정에서 동기화됩니다. 그룹은 최대 8개의 부모 그룹에 속할 수 있습니다.

### 그룹화된 컬렉션 안의 에셋이 자동으로 그룹에 속하나요?

아니요. 그룹 멤버십은 직접 멤버에만 저장됩니다. 컬렉션을 그룹에 추가하면 해당 컬렉션이 `group.collections`에 추가되고 컬렉션에 Groups 플러그인이 기록됩니다. 그 컬렉션에 민팅된 NFT는 자동으로 `group.assets`에 추가되지 않습니다.

### 스탠드얼론 에셋이 그룹의 직접 멤버가 될 수 있나요?

네. `addAssetsToGroup`을 사용해 컬렉션 없이 에셋을 `group.assets`에 직접 추가할 수 있습니다. 컬렉션 관리 에셋도 올바른 권한자가 서명하면 명시적으로 추가할 수 있습니다.

### 누가 그룹 멤버십을 변경할 수 있나요?

그룹 update authority가 추가/제거 명령에 서명합니다. 컬렉션 관리 에셋의 경우 컬렉션 update authority(또는 인가된 delegate)가 그룹을 대신해 해당 에셋을 추가하거나 제거할 수 있습니다.
