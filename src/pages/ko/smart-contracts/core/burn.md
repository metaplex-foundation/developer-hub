...
Asset은 `burn` 명령어를 사용하여 소각할 수 있습니다. 이렇게 하면 Asset 계정의 렌트 예치금이 소유자에게 반환됩니다. 계정이 다시 열리는 것을 방지하기 위해 매우 소액의 SOL(0.00089784)만 계정에 남습니다.
{% callout type="warning" title="렌트만 환불됩니다" %}
`burn`은 계정 데이터 크기에 대한 렌트 면제 잔액에서 1바이트 하한을 뺀 금액을 환불합니다. 주소 재오픈 공격을 방지하기 위해 계정은 열린 상태로 유지됩니다. 아직 회수되지 않은 프로토콜 수수료 등 렌트를 초과하는 lamports는 Metaplex 수수료 수집기가 회수할 때까지 닫힌 계정에 남아 있습니다.
{% /callout %}
{% totem %}
