...
{% quick-link title="MPL Token Metadata와의 차이점 개요" icon="AcademicCap" href="/ko/smart-contracts/core/tm-differences" description="Token Metadata에 익숙하고 새로운 기능이나 변경된 동작의 개요를 보고 싶나요?" /%}
...
- **강제 로열티**: Core는 기본적으로 [로열티 집행](/ko/smart-contracts/core/plugins/royalties)을 허용합니다.
- **일급 컬렉션 지원**: 자산을 [컬렉션](/ko/smart-contracts/core/collections)으로 그룹화할 수 있습니다. 이는 Token Metadata에서도 가능하지만, Core에서는 컬렉션이 자체 자산 클래스로서 다음과 같은 추가 기능을 허용합니다:
...
Metaplex Core가 무엇인지 개략적으로 살펴보았으므로, Core 자산과 상호 작용하는 데 사용할 수 있는 다양한 언어/프레임워크를 나열하는 [시작하기](/ko/smart-contracts/core/getting-started) 페이지를 확인하는 것을 권장합니다. [MPL Token Metadata와의 차이점](/ko/smart-contracts/core/tm-differences) 페이지도 살펴보는 것이 좋습니다. 그 이후에 다양한 기능 페이지를 사용하여 cNFT에서 수행할 수 있는 특정 작업에 대해 자세히 알아볼 수 있습니다.