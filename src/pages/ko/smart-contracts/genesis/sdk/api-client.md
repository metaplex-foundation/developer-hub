...
밈코인 출시의 경우, SDK는 예치 시작 시간만 필요한 간소화된 입력을 사용합니다. 할당 분할, LP 구성, 수수료 등 다른 모든 매개변수는 합리적인 기본값으로 하드코딩되어 있습니다.
...
할당, 유동성, 베스팅을 직접 제어하려면 `'project'`를 사용하세요. 최소한의 구성으로 빠르고 고정된 출시를 하려면 `'memecoin'`을 사용하세요.

**런칭 타입 읽기:** 생성 후 런칭 타입은 백엔드 크랭크에 의해 Genesis Account에 온체인으로 기록됩니다. 조회하려면 [`fetchGenesisAccountV2`](/smart-contracts/genesis/sdk/javascript#genesis-account)(온체인 SDK) 또는 REST API 응답의 [`type` 필드](/smart-contracts/genesis/integration-apis#shared-types)를 사용하세요. [GPA 빌더](/smart-contracts/genesis/sdk/javascript#gpa-builder--query-by-launch-type)는 타입별 모든 런칭 필터링을 지원합니다.
{% /callout %}
...
| `launchType` | `LaunchType` | 예 | `'project'` 또는 `'memecoin'` |
| `launch` | `ProjectLaunchInput \| MemecoinLaunchInput` | 예 | 런칭 구성 (`launchType`에 따라 필드가 다름) |
...
| `raiseGoal` | `number` | 최소 모금 목표 견적 토큰, 정수 단위 (예: 250 SOL). 최소 250 SOL 또는 5,000 USDC |
...
`launchType`이 `'memecoin'`인 경우, `launch` 객체는 예치 시작 시간만 필요합니다. 모든 할당 및 수수료 매개변수는 하드코딩되어 있습니다.

| 필드 | 타입 | 설명 |
|------|------|------|
| `depositStartTime` | `Date \| string` | 1시간 예치 기간이 시작되는 시점 |
