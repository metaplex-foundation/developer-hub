---
title: Presale
metaTitle: Genesis Presale | 솔라나 고정가 토큰 판매 | Metaplex
description: 솔라나에서 토큰 프리세일을 실행하세요. 사용자가 SOL을 예치하고 미리 정해진 비율로 SPL 토큰을 받는 고정가 토큰 판매. Genesis 런치패드를 통한 온체인 토큰 오퍼링.
created: '01-15-2025'
updated: '01-31-2026'
keywords:
  - presale
...
**Presale**은 솔라나에서 고정가 토큰 배포를 제공합니다. 할당량과 SOL 상한을 기반으로 SPL 토큰 가격을 미리 설정합니다. 사용자는 받을 수량을 정확히 알고, 여러분은 모금액을 정확히 파악할 수 있습니다. Genesis에서 "Presale"이란 초기 거래 직전에 토큰이 판매되는 것을 의미합니다. 구매자는 토큰을 직접 받으며, 미래에 토큰을 받을 권리가 아닙니다. {% .lead %}
...
3. 예치 기간 종료 후, `triggerBehaviorsV2`를 실행하여 종료 동작을 처리하고 자금을 이동합니다
...
Unlocked Bucket은 `triggerBehaviorsV2` 실행 후 Presale에서 SOL을 받습니다.
...
예치 기간 종료 후, `triggerBehaviorsV2`를 실행하여 Presale Bucket에 설정된 종료 동작을 처리합니다—여기서는 수집된 SOL을 Unlocked Bucket으로 이동합니다.

{% callout type="warning" title="예치 기간이 끝난 후 실행해야 합니다" %}
`triggerBehaviorsV2`는 Presale의 `depositEndCondition`이 충족된 후에만 호출할 수 있습니다. 이전에 호출하면 `PresaleBucketNotEnded` 오류가 발생합니다.
{% /callout %}

{% code-tabs-imported from="genesis/trigger_presale_v2" frameworks="umi" filename="triggerBehaviors" /%}

**이것이 중요한 이유:** 종료 동작을 트리거하지 않으면 수집된 SOL은 Presale Bucket에 잠긴 상태로 유지됩니다. 사용자는 토큰을 청구할 수 있지만 팀은 모금한 자금에 접근할 수 없습니다.
...
| **Trigger** | 예치 기간 종료 후 종료 동작을 처리하는 `triggerBehaviorsV2` 명령 |

## 다음 단계

- [Launch Pool](/ko/smart-contracts/genesis/launch-pool) - 유기적 가격 발견을 통한 공정한 출시
- [Uniform Price Auction](/ko/smart-contracts/genesis/uniform-price-auction) - 경매식 입찰 기반 할당
