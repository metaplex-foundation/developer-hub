...
- **Metaplex Agent API** — [`mintAndSubmitAgent`](/agents/create-agent)가 한 번의 API 호출로 Core 자산 생성과 신원 등록을 수행합니다 (권장)
...
{% callout title="경로 선택" %}
- **가장 빠른 방법?** [Mint Agent API](/agents/create-agent) 사용 — 한 번의 호출로 에이전트 생성 및 등록
- **수동 등록?** SDK 설정과 개별 명령은 [시작하기](/smart-contracts/mpl-agent/getting-started)를 참조하세요
