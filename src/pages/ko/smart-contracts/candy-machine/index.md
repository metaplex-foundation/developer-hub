...
- `name`: NFT의 이름.
- `uri`: NFT의 [JSON 메타데이터](/ko/smart-contracts/token-metadata/token-standard#the-non-fungible-standard)를 가리키는 URI. 이는 JSON 메타데이터가 이미 온체인(예: Arweave, IPFS) 또는 오프체인(예: AWS, 자체 서버) 스토리지 제공업체를 통해 업로드되었음을 의미합니다.
