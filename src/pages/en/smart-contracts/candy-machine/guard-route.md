...
You will find a detailed explanation of the route instruction of each guard that supports it and their underlying paths [on their respective pages](/smart-contracts/candy-machine/guards).

Let’s take a minute to illustrate how the route instruction works by providing an example. The [**Allow List**](/smart-contracts/candy-machine/guards/allow-list) guard, for instance, supports the route instruction in order to verify that the minting wallet is part of the preconfigured list of wallets.
...
The route instruction makes guards even more powerful by allowing them to ship with their own custom program logic. Check out the dedicated pages of [all available guards](/smart-contracts/candy-machine/guards) to see the full feature set of each guard.

Now that we know everything there is to know about setting up Candy Machines and their guards, it’s about time we talk about minting. See you on [the next page](/smart-contracts/candy-machine/mint)!
