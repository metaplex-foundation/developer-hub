---
title: Mint Agent
metaTitle: Agents - Mint Agent | REST API | Metaplex
...
- Stores the EIP-8004 metadata and a hosted [A2A AgentCard](/agents/api/get-agent-card) (yours, or synthesized from the metadata)
...
- A hosted A2A service entry is spliced into `services[]` so EIP-8004 consumers can discover the [AgentCard endpoint](/agents/api/get-agent-card); this is a no-op if you already authored one.
- The agent record is stored when you call this endpoint, but it only appears in [List Agents](/agents/api/list-agents) once the signed transaction has been confirmed and indexed.
