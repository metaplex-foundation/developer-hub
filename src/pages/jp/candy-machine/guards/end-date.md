...
description: "ミントを終了する日付を決定します。"
---

## 概要

**End Date**ガードは、ミントを終了する日付を指定します。この日付以降、ミントは許可されません。
...
End Dateガードには以下の設定が含まれます：

- **Date**: この日付以降、ミントが許可されない日付。

{% dialect-switcher title="End Dateガードを使用してCandy Machineを設定する" %}
...
APIリファレンス: [create](https://mpl-candy-machine.typedoc.metaplex.com/functions/create.html), [EndDate](https://mpl-candy-machine.typedoc.metaplex.com/types/EndDate.html)
...
_End Dateガードにはミント設定は必要ありません。_

## ルート命令

_End Dateガードはルート命令をサポートしません。_