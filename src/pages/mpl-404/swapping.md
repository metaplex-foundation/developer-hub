...
const assetsByCollection = await fetchAssetsByCollection(umi, COLLECTION, { skipDerivePlugins: false, })
const escrowAssets = assetsByCollection.filter(a => a.owner === publicKey(ESCROW) )
// WITH METADATA REROLLING, ANY ESCROW ASSET IS EFFECTIVELY THE SAME. WE CAN JUST USE THE FIRST ONE
// WITHOUT METADATA REROLLING A USER COULD CHOOSE TO RECIEVE A SPECIFIC ASSET
