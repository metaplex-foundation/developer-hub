...
Metaplex x402 是面向 LLM 推理、图像生成和 Solana RPC 的按请求付费 HTTP API — 无需 API 密钥、账户或订阅。只需准备一个持有 USDC 的 Solana 钱包、[Core 资产](/zh/smart-contracts/core)或[已注册的 Agent](/zh/agents/register-agent)，每个请求都会在发出时以 USDC 自动完成支付。{% .lead %}
...
- **三项服务** — OpenAI 兼容的聊天补全、OpenAI 兼容的图像生成，以及支持 [DAS](/zh/solana/rpcs-and-das) 透传的 Solana JSON-RPC
- **三种支付模式** — 标准钱包、Core 资产或 Agent 直接支付，以及无需逐次签名即可支付的[委托 Agent](/zh/agents/x402/payment-modes#pay-instantly-with-a-delegated-agent)
...
聊天和图像端点使用标准的 OpenAI 传输格式，因此任何 OpenAI 兼容客户端只需修改 `baseURL` 并提供支持支付的 `fetch` 即可工作。请求与响应的完整细节见 [API 参考](/zh/agents/x402/api-reference)。

## Mech：运营 Metaplex x402 的 Agent {% #mech-the-agent-that-operates-metaplex-x402 %}

Mech 是销售 x402 服务的链上 Metaplex Agent，USDC 支付会结算到 Mech 的 Core 钱包。Mech 本身也是一个拥有 [Asset Signer PDA 钱包](/zh/smart-contracts/core/execute-asset-signing)的[已注册 Agent](/zh/agents/what-is-an-agent) — 即一个 Agent 向您的 Agent 销售服务，这正是[Agent 商业](/zh/agents/agent-commerce)模式端到端运转的样子。
...
安装客户端，并用支持支付的 `fetch` 将 OpenAI SDK 指向 Metaplex x402。本示例使用标准钱包支付；三种模式的完整说明见[支付模式指南](/zh/agents/x402/payment-modes)。
...
委托是一项可随时撤销的链上授权。各模式的设置、代码和撤销流程见[支付模式指南](/zh/agents/x402/payment-modes)。
...
| **Asset Signer PDA** | 由 `["mpl-core-execute", asset]` 派生的 MPL Core PDA — Core 资产的链上钱包，通过 Core 的 [Execute 生命周期钩子](/zh/smart-contracts/core/execute-asset-signing)控制 |
...
| **DAS** | [Digital Asset Standard](/zh/solana/rpcs-and-das) 读取 API，可通过 x402 的 RPC 端点透传使用 |
