...
{% quick-link title="注册 Agent" icon="InboxArrowDown" href="/zh/agents/register-agent" description="铸造 Core 资产并将其注册到 Metaplex Agent Registry 的分步指南。" /%}

{% quick-link title="Metaplex Skill" icon="CodeBracketSquare" href="/zh/agents/skill" description="为您的编码 Agent 提供 Metaplex 程序的最新知识。" /%}
...
**钱包创建与充值** — 生成主钱包，并充值至少 0.2 SOL 以支付 [Core](/zh/core) 资产注册费和持续的交易费用。

**Agent 注册** — 铸造一个 [Core](/zh/core) 资产作为 Agent 的链上身份，其元数据符合 EIP-8004 Agent 标准。此过程生成 Agent 的 Core 资产地址 — 所有后续操作均需此地址。

**钱包激活** — 为 Asset Signer PDA 充值并激活，该运营钱包供 Agent 自主提交交易使用。

**委托（可选）** — 授权独立的 executor 钱包代表 Agent 提交交易。

**代币发行（可选）** — 通过 [Genesis](/zh/smart-contracts/genesis) 使用 LaunchPool（48 小时存款窗口，最低募集 250 SOL 或 25,000 USDC）或 Bonding Curve（即时交易，无最低要求）创建代币。
...
- 注册生成的 Core 资产地址是[创建 Agent 代币](/zh/agents/create-agent-token)、[Agent 商业](/zh/agents/agentic-commerce)及其他 Agent 工作流的必要条件
