...
- 覆盖六个程序：[Agent Registry](/agents)、[Genesis](/smart-contracts/genesis)、[Core](/smart-contracts/core)、[Token Metadata](/smart-contracts/token-metadata)、[Bubblegum](/smart-contracts/bubblegum-v2)、[Candy Machine](/smart-contracts/core-candy-machine)
- 支持 CLI、Umi SDK 和 Kit SDK 方法
- 适用于 Claude Code、Cursor、Copilot、Codex、Windsurf 及其他兼容代理
...
Skill 覆盖六个 Metaplex 程序及其完整操作集：

| 程序 | 用途 | CLI | Umi SDK | Kit SDK |
|---------|---------|-----|---------|---------|
| **[Agent Registry](/agents)** | 链上代理身份、钱包和执行委托 | Yes | Yes | — |
| **[Genesis](/smart-contracts/genesis)** | 通过 launchpool 或 bonding curve 代币发行与 Raydium 毕业 | Yes | Yes | — |
...
- **CLI (`mplx`)** — 从终端直接执行 Metaplex 操作。代理注册（`mplx agents`）、代币发行和 bonding curve 创建（`mplx genesis`）、资产创建、上传、Candy Machine 部署、树创建、转移等。
- **Umi SDK** — 覆盖所有程序的完整编程访问。代理身份和委托、Genesis 发行和 bonding curve 交换、按所有者/集合/创建者查询、DAS API 查询、委托管理和插件配置。
...
| 技能格式 | [Agent Skills](https://agentskills.io) |
| CLI 包 | [`@metaplex-foundation/cli`](https://github.com/metaplex-foundation/cli) (`mplx`) |
| 覆盖程序 | 6个（Agent Registry、Genesis、Core、Token Metadata、Bubblegum、Candy Machine） |
...
| 术语 | 定义 |
|------|-----------|
| Agent Skill | 为 AI 编码代理提供特定领域准确上下文的结构化知识库 |
| 渐进式披露 | 代理首先读取轻量级路由器，然后仅加载当前任务所需的参考文件的架构 |
| SKILL.md | 将任务映射到特定参考文件的路由器文件 |
| `mplx` | 提供所有支持程序直接终端访问的 Metaplex CLI 工具 |
| Umi SDK | 提供所有程序编程访问的 Metaplex 主要 TypeScript SDK 框架 |
| Kit SDK | 使用 `@solana/kit` 的轻量级替代 SDK，目前仅支持 Token Metadata |
