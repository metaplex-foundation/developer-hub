---
title: 按集合获取 Core 资产
metaTitle: 按集合获取 Core 资产 | DAS API Core 扩展
description: 返回给定集合中所有 MPL Core 资产的信息
---

返回具有给定权限的所有 MPL Core 资产的信息，包括其元数据、所有者和解析的插件数据，以及从集合继承的数据。

## 代码示例

将 `<ENDPOINT>` 替换为您的个人 RPC，将 `<PublicKey>` 值替换为您要获取 NFT 的集合的公钥。
...
此示例数组中只有一个对象。对于拥有多个 Core 资产的集合，数组将包含更多条目。
