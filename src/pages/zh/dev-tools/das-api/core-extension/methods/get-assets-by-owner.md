---
title: 按所有者获取 Core 资产
metaTitle: 按所有者获取 Core 资产 | DAS API Core 扩展
description: 返回按所有者获取的所有 MPL Core 资产的信息
---

返回给定公钥拥有的所有 MPL Core 资产的信息，包括其元数据、所有者和解析的插件数据，以及从集合继承的数据。

## 代码示例

将 `<ENDPOINT>` 替换为您的个人 RPC，将 `<PublicKey>` 值替换为所有者的公钥。
...
## 示例响应
此示例数组中只有一个对象。对于拥有多个 Core 资产的公钥，数组将包含更多条目。
