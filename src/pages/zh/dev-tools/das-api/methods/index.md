...
- [`getAsset`](/zh/dev-tools/das-api/methods/get-asset): 返回压缩/标准资产的信息，包括元数据和所有者。`MplCoreAsset` 响应可能包含代理字段（`is_agent`、`asset_signer`、`agent_token`）。
- [`getAssets`](/zh/dev-tools/das-api/methods/get-assets): 返回多个压缩/标准资产的信息，包括元数据和所有者。每个项目使用与 `getAsset` 相同的资产形状；代理字段仅可能出现在 `MplCoreAsset` 行上。
...
- [`searchAssets`](/zh/dev-tools/das-api/methods/search-assets): 根据搜索条件返回资产列表。支持代理过滤器（`isAgent`、`agentToken`、`assetSigner`）——请参阅[读取代理数据](/zh/agents/read-agent-data#read-agent-data-via-das-api)。
