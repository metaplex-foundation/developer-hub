...
- **一步完成**：通过 `--agentMint` 和 `--agentSetToken` 在创建联合曲线发行时关联 Agent
...
创建 Agent 代币最简单的方式是在创建发行时传入 `--agentMint`。这将从 Agent 的 [Asset Signer PDA](/dev-tools/cli/config/asset-signer-wallets) 自动推导创作者费钱包，并可选择在同一交易中关联代币。
