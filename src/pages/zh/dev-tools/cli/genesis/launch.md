...
faqs:
  - q: What is the difference between genesis launch create and the manual flow?
    a: genesis launch create 命令是一体化流程，调用 Genesis API 构建交易、签名并发送，然后在 Metaplex 平台上注册发行——全部在一个命令中完成。手动流程需要分别执行 create、bucket、finalize 和 register 步骤。
  - q: When should I use genesis launch register?
    a: 当您已经使用低级 CLI 命令（genesis create、bucket add-launch-pool 等）创建了 Genesis 账户，并希望在 Metaplex 平台上注册以获得公共发行页面时，请使用 genesis launch register。
  - q: What network does the launch command use?
    a: 网络从您配置的 RPC 端点自动检测。您可以使用 --network 参数覆盖（solana-mainnet 或 solana-devnet）。
---
...
- **`genesis launch register`**：在 Metaplex 平台上注册现有的 Genesis 账户以获得公共发行页面
- **metaplex.com 兼容**：通过 API 创建或注册的发行将在 [metaplex.com](https://metaplex.com) 上显示公共发行页面
- **总供应量**：目前固定为 1,000,000,000 个代币
- **存款期**：目前为 48 小时

## 不在范围内

手动 Genesis 账户创建、单独的 bucket 配置、Presale 设置、前端开发。

**跳转到：** [Launch Create](#launch-create) · [Launch Register](#launch-register) · [锁定分配](#locked-allocations) · [常见错误](#common-errors) · [常见问题](#faq)
...
| `--tokenAllocation <integer>` | Launch Pool 代币分配量（10 亿总供应量的一部分） |
| `--depositStartTime <string>` | 存款开始时间（ISO 日期字符串或 Unix 时间戳） |
| `--raiseGoal <integer>` | 募集目标（整数单位，例如 200 表示 200 SOL） |
...
| `--quoteMint <string>` | 报价代币铸造（目前支持 `SOL` 或 `USDC`） | `SOL` |
...
`mplx genesis launch register` 命令在 Metaplex 平台上注册现有的 Genesis 账户。当您使用低级 CLI 命令（`genesis create`、`bucket add-launch-pool` 等）创建了 Genesis 账户并希望获得公共发行页面时使用此命令。

```bash {% title="注册 Genesis 账户" %}
...
| 参数 | 描述 | 必需 | 默认值 |
|------|-------------|----------|---------|
| `--launchConfig <path>` | 发行配置的 JSON 文件路径 | 是 | — |
...
锁定分配允许您保留部分代币供应并设置归属计划。通过 `--lockedAllocations` 提供 JSON 数组文件。
...
| 错误 | 原因 | 修复方法 |
|-------|-------|-----|
| API request failed | 网络问题或无效输入 | 检查错误响应详情——命令在验证错误时会显示 API 响应体 |
| Locked allocations file not found | 文件路径错误 | 验证分配 JSON 文件的路径 |
| Must contain a JSON array | 分配文件不是数组 | 确保 JSON 文件包含数组 `[...]`，而不是对象 |
| raydiumLiquidityBps out of range | 值不在 2000-10000 范围内 | 使用 2000（20%）到 10000（100%）之间的值 |
...
`genesis launch create` 命令是一体化流程，调用 Genesis API 构建交易、签名并发送，然后在 Metaplex 平台上注册发行——全部在一个命令中完成。手动流程需要分别执行 `create`、`bucket add-launch-pool`、`finalize` 和 register 步骤。

**什么时候应该使用 `genesis launch register`？**
当您已经使用低级 CLI 命令（`genesis create`、`bucket add-launch-pool` 等）创建了 Genesis 账户，并希望在 Metaplex 平台上注册以获得公共发行页面时使用。

**launch 命令使用什么网络？**
网络从您配置的 RPC 端点自动检测。您可以使用 `--network` 参数覆盖（`solana-mainnet` 或 `solana-devnet`）。
