...
faqs:
  - q: How are tokens distributed in a launch pool?
    a: 代币按比例分配。如果您存入了池中总报价代币的 10%，您将获得该 bucket 基础代币分配的 10%。
  - q: Can I withdraw after depositing?
    a: 可以，但仅限存款期间。存款窗口关闭后，将无法提取。
  ...
  - q: What is a claim schedule?
    a: Claim schedule 为代币领取添加归属机制——代币随时间逐步释放，而不是一次性全部释放，并可设置可选的锁定期。
---

{% callout title="您将完成的操作" %}
从 CLI 运行完整的 Launch Pool 生命周期：
- 添加带有分配量和时间窗口的 Launch Pool bucket
- 配置可选的罚金、归属和白名单
- 存入、提取、转换和领取代币
{% /callout %}
...
Launch Pool 在窗口期内收集存款并按比例分配代币。本页涵盖完整的 Launch Pool 生命周期——从创建 bucket 到领取代币。

- **分配方式**：按比例——您的存款份额决定您的代币份额
- **命令**：`bucket add-launch-pool`、`deposit`、`withdraw`、`transition`、`claim`
- **可选功能**：End behavior、存入/提取罚金、奖励计划、领取归属、白名单
...
**跳转到：** [添加 Bucket](#add-launch-pool-bucket) · [存入](#deposit) · [提取](#withdraw) · [转换](#transition) · [领取](#claim) · [完整生命周期](#full-lifecycle-example) · [常见错误](#common-errors) · [常见问题](#faq)

## 添加 Launch Pool Bucket

`mplx genesis bucket add-launch-pool` 命令向 Genesis 账户添加 Launch Pool bucket。

```bash {% title="添加 Launch Pool bucket" %}
...
| `--minimumDeposit <string>` | | 每笔交易的最低存入额（基本单位） | 否 |
| `--depositLimit <string>` | | 每用户最高存入额（基本单位） | 否 |
| `--minimumQuoteTokenThreshold <string>` | | bucket 成功所需的最低报价代币总量 | 否 |
...
2. 带 end behavior 和最低存入额：
```bash {% title="带 end behavior" %}
mplx genesis bucket add-launch-pool <GENESIS_ADDRESS> \
  --allocation 500000000000000 \
  --depositStart 1704067200 \
  --depositEnd 1704153600 \
...
`mplx genesis deposit` 命令在存款窗口期间将报价代币存入 Launch Pool bucket。如果使用 SOL 作为报价代币，请先进行包装。

```bash {% title="存入 Launch Pool" %}
...
| 参数 | 缩写 | 描述 | 必需 |
|------|-------|-------------|----------|
| `--amount <string>` | `-a` | 要提取的报价代币数量（基本单位） | 是 |
| `--bucketIndex <integer>` | `-b` | Launch Pool bucket 的索引（默认：0） | 否 |

## 转换

`mplx genesis transition` 命令在存款期关闭后执行 end behavior，将收集的报价代币转移到目标 bucket。

```bash {% title="转换 end behavior" %}
...
- 必须在存款期结束后调用
- 仅在 bucket 配置了 end behavior 时需要

## 领取

`mplx genesis claim` 命令从 Launch Pool bucket 领取基础代币。用户收到的代币与其存款成比例。

```bash {% title="从 Launch Pool 领取" %}
...
```bash {% title="完整 Launch Pool 生命周期" %}
# 1. 创建 Genesis 账户
mplx genesis create \
  --name "My Token" \
  --symbol "MTK" \
...
# 3. 添加带 end behavior 的 Launch Pool bucket
...
| Withdrawal period ended | 在存款窗口关闭后尝试提取 | 提取仅在存款期间可用 |
...
| End behavior not configured | 在未配置 end behavior 的 bucket 上运行 `transition` | 仅配置了 `--endBehavior` 的 bucket 才需要转换 |
| Deposit period not ended | 在存款关闭前运行 `transition` | 等待 `depositEnd` 时间戳之后 |

## 常见问题

**Launch Pool 中代币如何分配？**
代币按比例分配。如果您存入了池中总报价代币的 10%，您将获得该 bucket 基础代币分配的 10%。

**存入后可以提取吗？**
可以，但仅限存款期间。存款窗口关闭后，将无法提取。

**什么是 end behavior？**
End behavior 在存款期结束后将收集的报价代币从 Launch Pool 转发到目标 bucket（通常是 unlocked bucket）。您必须调用 `genesis transition` 来执行它们。

**什么是 claim schedule？**
Claim schedule 为代币领取添加归属机制。代币不是一次性全部获得，而是根据配置的 `period`、`cliffTime` 和 `cliffAmountBps` 逐步释放。

**如果 minimumQuoteTokenThreshold 未达到会怎样？**
如果总存款未达到阈值，bucket 将不会成功，存款人可以取回他们的资金。

**可以将 end behavior 拆分到多个目标吗？**
可以。多次指定 `--endBehavior`，使用不同的目标地址和百分比（以基点表示，总计 10000）。

## 术语表

| 术语 | 定义 |
|------|------------|
| **Launch Pool** | 根据存款份额按比例分配代币的 bucket 类型 |
| **End Behavior** | 在存款关闭后将收集的报价代币转发到目标 bucket 的规则 |
| **Transition** | 执行 end behavior 的命令——必须在存款期后显式调用 |
| **Claim Schedule** | 控制代币随时间逐步释放的归属配置 |
| **Deposit Penalty** | 对存款收取的费用，以基点配置，可带有可选的基于时间的斜率 |
| **Withdraw Penalty** | 在存款期间对提取收取的费用 |
| **Bonus Schedule** | 对早期或特定时间存款的额外代币分配 |
| **Allowlist** | 基于默克尔树的访问控制，限制谁可以存入 |
