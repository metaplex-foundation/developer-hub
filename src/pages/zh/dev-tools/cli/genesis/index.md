...
description: 使用 Metaplex CLI (mplx) 进行 Genesis 代币发行的 CLI 命令概述。
keywords:
  - Genesis CLI
  - token launch CLI
  - mplx genesis
...
programmingLanguage:
  - Bash
faqs:
  - q: 什么是 mplx genesis 命令？
    a: mplx genesis 命令组允许您从终端运行完整的 Genesis 代币发行——创建账户、配置 bucket、存入、领取和撤销权限。
  - q: Genesis 中有哪些不同的 bucket 类型？
    a: Genesis 有三种 bucket 类型——Launch Pool（基于存款的按比例分发）、Presale（固定价格代币销售）和 unlocked（团队/国库分配，可直接领取）。
  - q: 存入前需要包装 SOL 吗？
    a: 是的。如果使用 SOL 作为报价代币，请先使用 mplx toolbox sol wrap 包装，然后再存入任何 bucket。
  - q: 可以撤销 finalize 操作吗？
    a: 不可以。Finalize 是不可逆的。一旦 finalize，就不能再添加 bucket，配置将被锁定。
  - q: 代币数量如何指定？
    a: 所有数量均以基本单位表示。对于 9 位小数，1,000,000 个代币 = 1000000000000000 基本单位。存款金额使用报价代币的基本单位（SOL 的 lamports）。
---
...
`mplx genesis` 命令允许您从终端运行完整的 Genesis 代币发行——创建账户、配置 bucket、存入、领取和撤销权限。

- **工具**：Metaplex CLI (`mplx`) 的 `genesis` 命令组
- **Bucket 类型**：Launch Pool（按比例）、Presale（固定价格）、unlocked（国库）
- **报价代币（手动流程）**：默认为 Wrapped SOL，支持任何 SPL 代币 mint 地址
...
- 用于交易费用的 SOL
- 通过 `mplx config rpc add` 配置的 RPC 端点，或使用 `-r` 参数传入
...
1. **创建** — `genesis create` 设置 Genesis 账户和代币 mint。
2. **添加 Bucket** — 添加一个或多个 bucket 来定义代币的分发方式。使用 `bucket add-launch-pool` 进行按比例分发，使用 `bucket add-presale` 进行固定价格销售，或使用 `bucket add-unlocked` 进行团队/国库分配。
...
5. **提取**（可选） — 用户可以在存款期间使用 `genesis withdraw` 从 Launch Pool 中提取。
6. **转换**（可选） — 如果 Launch Pool 设置了 end behavior，在存款关闭后调用 `genesis transition` 将收集的代币转发到目标 bucket。
7. **领取** — 领取期开始后，用户使用 `genesis claim` 或 `genesis presale claim` 领取基础代币。国库钱包使用 `genesis claim-unlocked`。
8. **撤销**（可选） — `genesis revoke` 永久撤销代币的 mint 和/或 freeze 权限。

如果您使用了手动流程并希望获得公开的发行页面，请使用 `genesis launch register` 在 Metaplex 平台上注册您的 Genesis 账户。
...
| `genesis revoke` | 撤销 mint/freeze 权限 |
...
- `totalSupply` 和 `allocation` 以基本单位表示——对于 9 位小数，`1000000000000000` = 1,000,000 个代币
- 存款和提取金额以报价代币的基本单位表示（SOL 的 lamports，其中 1 SOL = 1,000,000,000 lamports）
- 如果使用 SOL 作为报价代币，请先使用 `mplx toolbox sol wrap <amount>` 包装
...
| Genesis already finalized | 在 `finalize` 后尝试添加 bucket | Finalize 是不可逆的——如果配置错误请创建新的 Genesis 账户 |
| Allocation exceeds total supply | Bucket 分配总和超过 `totalSupply` | 减少分配使其总和不超过 `totalSupply` |
| Deposit period not active | 在存款窗口外存入 | 使用 `genesis bucket fetch` 检查时间戳——存款仅在 `depositStart` 和 `depositEnd` 之间有效 |
| Claim period not active | 在领取窗口开启前领取 | 等待 `claimStart` 时间戳之后 |
...
Genesis 有三种 bucket 类型：**Launch Pool**（基于存款的按比例分发）、**Presale**（固定价格代币销售）和 **unlocked**（团队/国库分配，可直接领取）。

**存入前需要包装 SOL 吗？**
是的。如果使用 SOL 作为报价代币，请先使用 `mplx toolbox sol wrap <amount>` 包装，然后再存入任何 bucket。

**可以撤销 finalize 操作吗？**
不可以。Finalize 是不可逆的。一旦 finalize，就不能再添加 bucket，配置将被锁定。在运行 `genesis finalize` 之前请仔细检查所有内容。

**代币数量如何指定？**
所有数量均以基本单位表示。对于 9 位小数，1,000,000 个代币 = `1000000000000000` 基本单位。存款金额使用报价代币的基本单位（SOL 的 lamports，其中 1 SOL = 1,000,000,000 lamports）。
...
| 术语 | 定义 |
|------|------------|
| **Genesis Account** | 管理整个代币发行的 PDA——持有配置、bucket 引用和 mint 权限 |
| **Bucket** | Genesis 发行中的分发通道，定义代币的一部分如何分配和分发 |
| **Launch Pool** | 在窗口期收集存款并按比例向存款人分发代币的 bucket 类型 |
| **Presale** | 以由 `quoteCap / allocation` 决定的固定价格出售代币的 bucket 类型 |
| **Unlocked Bucket** | 用于团队/国库的 bucket 类型——指定的接收者可以直接领取代币或转发的报价代币 |
| **Quote Token** | 用户存入的代币（通常是 Wrapped SOL），用于交换基础代币 |
| **Base Token** | 正在发行和分发给存款人的代币 |
| **Base Units** | 代币的最小单位——对于 9 位小数，1 个代币 = 1,000,000,000 基本单位 |
| **End Behavior** | 在存款期结束后将收集的报价代币从 Launch Pool 转发到目标 bucket 的规则 |
| **Finalize** | 锁定 Genesis 配置并激活发行的不可逆操作 |
| **Claim Schedule** | 控制领取期开始后代币如何随时间释放的归属规则 |
