...
- `name`:NFT的名称。
- `uri`:指向NFT的[JSON元数据](/zh/smart-contracts/token-metadata/token-standard#the-non-fungible-standard)的URI。这意味着JSON元数据已经通过链上(例如Arweave、IPFS)或链下(例如AWS、您自己的服务器)存储提供商上传。
