...
通过 sugar 允许铸造的前提是您的 Candy Machine 创建时**没有附加守卫**。要使用 sugar 创建 Candy Machine，您可以按照[此](/zh/smart-contracts/candy-machine/guides/create-an-nft-collection-on-solana-with-candy-machine)指南的第一步进行操作。如果您的 Candy Machine 已附加守卫，可以使用 `sugar guard remove` 移除它们。
