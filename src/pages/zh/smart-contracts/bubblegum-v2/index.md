...
- **版税强制执行**：由于Bubblegum V2使用[MPL-Core](/zh/smart-contracts/core)集合，可以使用`ProgramDenyList`等方式对cNFT强制执行版税。
- **继承版税**：铸造到 MPL-Core 集合的 cNFT 可以在叶子上存储哨兵 seller fee basis points 值（`65535`），并继承集合的 Royalties 插件配置，而无需在每次铸造时复制 basis points。
