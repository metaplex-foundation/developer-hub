...
faqs:
  ...
  - q: 铸造 Core Asset 需要多少费用？
    a: 每个基础资产约 0.003 SOL（含 0.0015 SOL Core 协议费用），而 Token Metadata 约为 0.022 SOL。这使 Core 的铸造成本降低约 80%。
  - q: 哪些 RPC 提供商支持 Core？
...
- 单账户设计：每次铸造约 0.003 SOL，含 0.0015 SOL 协议费用（Token Metadata 为 0.022 SOL）
- 默认强制版税，带白名单/黑名单控制
...
| 标准 | 铸造成本 | 计算单元 |
|----------|-----------|---------------|
| **Metaplex Core** | ~0.003 SOL（含 0.0015 SOL 协议费用） | ~17,000 CU |
...
每个基础资产约 0.003 SOL（含 0.0015 SOL Core 协议费用），而 Token Metadata 约为 0.022 SOL。这使 Core 的铸造成本降低约 80%。更多详情请参阅[与 Token Metadata 的区别](/zh/smart-contracts/core/tm-differences)。
### 哪些 RPC 提供商支持 Core？
