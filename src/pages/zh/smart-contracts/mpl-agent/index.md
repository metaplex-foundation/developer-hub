...
- **Metaplex Agent API** — [`mintAndSubmitAgent`](/agents/create-agent) 通过一次 API 调用创建 Core 资产并注册身份（推荐）
...
- **最快方式？** 使用 [Mint Agent API](/agents/create-agent) — 一次调用即可创建并注册代理
- **手动注册？** 参阅[入门指南](/smart-contracts/mpl-agent/getting-started)了解 SDK 设置和各指令详情
