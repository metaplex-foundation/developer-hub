---
title: 绑定曲线 — 工作原理
metaTitle: Genesis 绑定曲线 — 工作方式 | Metaplex
description: Genesis 绑定曲线的工作原理 — 恒定乘积定价、虚拟储备金、费用结构、首次购买机制、生命周期阶段以及向 Raydium 流动性池的毕业。
keywords:
  - bonding curve
  - bonding curve v2
...
Genesis 绑定曲线是一种恒定乘积自动做市商，旨在完全售罄其代币供应并毕业进入 Raydium CPMM 池。 {% .lead %}

## Summary

Genesis 绑定曲线使用 `x × y = k` 恒定乘积定价模型和虚拟储备金，在 Solana 上实现确定性且持续可用的代币发行。
...
Genesis 绑定曲线使用与 Uniswap V2 和 Raydium 相同的 `x × y = k` 公式。任意时刻的价格由代币储备金与 SOL 储备金的比率决定。
...
Genesis 绑定曲线通过用**虚拟储备金**（仅存在于定价数学中的虚拟 SOL 和虚拟代币）为曲线播种来解决这个问题。
...
- 绑定曲线与[发行池](/smart-contracts/genesis/launch-pool)和[预售](/smart-contracts/genesis/presale)是不同的发行类型
...
### Genesis 绑定曲线为什么使用虚拟储备金而不是标准 AMM？
...
| 术语 | 定义 |
|------|------|
| **恒定乘积 AMM** | 使用 `x × y = k` 保持储备金乘积不变的自动做市商。Uniswap V2、Raydium 和 Genesis 绑定曲线均使用 |
