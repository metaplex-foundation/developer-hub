---
title: Presale
metaTitle: Genesis Presale | 固定价格代币销售 Solana | Metaplex
description: 在 Solana 上进行代币预售。固定价格 SPL 代币销售，用户存入 SOL 并以预定汇率获得代币。使用 Genesis 发射台进行链上代币发售。
created: '01-15-2025'
updated: '01-31-2026'
keywords:
  - presale
...
3. 存款期结束后，运行 `triggerBehaviorsV2` 处理结束行为并转移资金
4. 用户根据存款金额领取代币
...
Unlocked Bucket在 `triggerBehaviorsV2` 执行后从Presale接收SOL。
...
存款期结束后，运行 `triggerBehaviorsV2` 处理Presale Bucket上配置的结束行为——此处将收集的SOL转移到Unlocked Bucket。

{% callout type="warning" title="必须等待存款期结束" %}
...
| **Trigger** | 存款期结束后处理结束行为的 `triggerBehaviorsV2` 指令 |

## 下一步

- [Launch Pool](/zh/smart-contracts/genesis/launch-pool) - 公平发射与有机价格发现
- [Uniform Price Auction](/zh/smart-contracts/genesis/uniform-price-auction) - 拍卖式基于出价的分配
