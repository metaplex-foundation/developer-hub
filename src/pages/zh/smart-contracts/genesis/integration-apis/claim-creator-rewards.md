...
当钱包没有可认领内容时，端点返回 HTTP `400` 和消息 `No rewards available to claim`——它**不会**返回带 `transactions: []` 的 `200`。调用方必须捕获错误（或检查 `response.status` 和 `body.error.message`），并将此情况视为"无事可做"，而非失败。SDK 将其呈现为类型化的 `GenesisApiError`；请参阅[错误处理](/smart-contracts/genesis/creator-fees#handling-the-no-rewards-case)。
{% /callout %}
