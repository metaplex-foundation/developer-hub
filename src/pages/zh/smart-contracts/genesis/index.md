...
发行类型在创建后由后端 crank 记录到 [Genesis Account](#genesis-account) 的链上数据中。交易者和聚合器可以通过 [JavaScript SDK](/smart-contracts/genesis/sdk/javascript#genesis-account)（`fetchGenesisAccountV2`）或 [Metaplex API](/zh/api)（REST 响应中的 `type` 字段）以编程方式查询类型。
