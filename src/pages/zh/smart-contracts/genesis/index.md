...
faqs:
  ...
  - q: Genesis 支持哪些发行类型？
    a: Genesis 支持多种发行类型，代表底层机制——Launch Pool（公平发射，按比例分配与价格发现）和 Presale（固定价格代币销售）。
---
...
发行类型在创建后由后端 crank 记录到 [Genesis Account](#genesis-account) 的链上数据中。交易者和聚合器可以通过 [JavaScript SDK](/smart-contracts/genesis/sdk/javascript#genesis-account)（`fetchGenesisAccountV2`）或 [Integration APIs](/api)（REST 响应中的 `type` 字段）以编程方式查询类型。
...
| **发行类型** | 发行的底层机制：`launchpool` 或 `presale`。创建后由后端 crank 在链上设置 |
