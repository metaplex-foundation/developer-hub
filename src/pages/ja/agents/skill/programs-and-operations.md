...
**CLI** (`mplx agents`)：エージェントアイデンティティの登録、実行の委任と取り消し、エージェントデータの取得、Genesisトークンのエージェントへのリンク。完全なエージェントトークン作成フローには、`mplx genesis launch create --agentAsset --agentSetToken`を使用してローンチとリンクを一括で実行。
