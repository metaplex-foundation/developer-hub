...
{% callout title="ミームコインローンチ" %}
ミームコインローンチの場合、SDK は設定の大部分をスキップする合理化されたパスを提供します。`launchType: 'memecoin'` と入金開始時刻を設定するだけで、アロケーション、LP、手数料、ロックスケジュールはすべてハードコードされています。詳細は [API クライアントのミームコインセクション](/ja/smart-contracts/genesis/sdk/api-client#memecoin-launch--simplified-flow)をご覧ください。
{% /callout %}
...
- **Launch Pool：** ユーザーが SOL を入金し、0% の手数料で引き出し可能
- **Presale：** ユーザーが固定価格で SOL を入金（ユーザーごとの入金上限（各ユーザーが貢献できる最大額）あり）

すべての入金には 0% のプロトコル手数料が適用されます。
