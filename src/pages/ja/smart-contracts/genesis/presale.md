...
3. 入金期間終了後、`triggerBehaviorsV2` を実行して End Behavior を処理し、資金を移動する
...
Unlocked bucket は `triggerBehaviorsV2` 実行後に Presale から SOL を受け取ります。
...
入金期間終了後、`triggerBehaviorsV2` を実行して Presale bucket に設定された End Behavior を処理します。ここでは収集した SOL を Unlocked bucket に移動します。

{% callout type="warning" title="入金期間終了後に実行してください" %}
`triggerBehaviorsV2` は Presale の `depositEndCondition` が満たされた後でのみ呼び出せます。早く呼び出すと `PresaleBucketNotEnded` エラーが発生します。
{% /callout %}

{% code-tabs-imported from="genesis/transition_presale_v2" frameworks="umi" filename="triggerBehaviors" /%}

**これが重要な理由：** End Behavior をトリガーしないと、収集した SOL は Presale bucket にロックされたままになります。ユーザーはトークンを請求できますが、チームは調達した資金にアクセスできません。
...
| **Trigger** | 入金期間終了後に End Behavior を処理する `triggerBehaviorsV2` インストラクション |
