...
faqs:
  ...
  - q: トークンのローンチタイプを取得するにはどうすればいいですか？
    a: トークンのミントアドレスを使用してfetchGenesisAccountV2FromSeedsでGenesisAccountV2オンチェーンアカウントを取得します。launchTypeフィールドは0（未初期化）、1（プロジェクト）、または2（ミーム）を返します。GPAビルダーを使用して特定のタイプの全ローンチを照会することもできます。
...
[Integration APIs](/smart-contracts/genesis/integration-apis)では文字列（`'project'`、`'memecoin'`、`'custom'`）として返されますが、オンチェーンSDKでは上記の数値列挙型を使用します。
...
トークンのミントアドレスを使用して`fetchGenesisAccountV2FromSeeds()`で`GenesisAccountV2`アカウントを取得します。`launchType`フィールドは`0`（未初期化）、`1`（プロジェクト）、`2`（ミーム）を返します。特定のタイプの全ローンチを照会するには、[GPAビルダー](#gpaビルダー--ローンチタイプで照会)を使用します。または、[Integration APIs](/smart-contracts/genesis/integration-apis)がREST応答で文字列としてローンチタイプを返します。
