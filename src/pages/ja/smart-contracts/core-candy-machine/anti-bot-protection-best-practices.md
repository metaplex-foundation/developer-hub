---
title: Core Candy Machineのボット対策ベストプラクティス
metaTitle: ボット対策ベストプラクティス | Core Candy Machine
...
updated: '03-10-2026'
faqs:
  - q: Candy Machineがガードを使用しているのに、なぜプレースホルダーメタデータが必要ですか？
    a: ガードはミントプロセスを保護しますが、すべてのCandy Machineデータはオンチェーンで公開されています。プレースホルダーなしでは、ボットが実際のメタデータURIを事前取得し、希少性トレイトを分析し、ミント開始前に特定のインデックスをターゲットにできます。プレースホルダーメタデータはリビールまでトレイト情報を隠します。
  - q: ボット対策でHidden SettingsとConfig Linesの違いは何ですか？
    a: Hidden Settingsはアセットを順次ミントし（インデックス0、1、2、...）、ミント順序が予測可能になります。Config Linesでは事前ランダム化された順序でプレースホルダーメタデータをロードでき、ミント後のリビールマッピングに加えて追加の予測不可能性の層を提供します。
  - q: ボットはバックエンドをリバースエンジニアリングしてThird-Party Signerガードをバイパスできますか？
    a: Third-Party Signerガードはバックエンドサーバーにのみ存在する秘密鍵からの暗号署名を要求します。攻撃者がフロントエンドをリバースエンジニアリングしても、秘密鍵へのアクセスなしにこの署名を偽造することはできません。これが、秘密鍵をクライアントサイドのコードに公開してはならない理由です。
  - q: プロジェクトにどのリビール戦略を選択すべきですか？
    a: 即座のユーザー満足とサポート負担の軽減にはインスタントリビールを選択してください。コミュニティ全体の興奮とよりシンプルなミントインフラストラクチャにはイベントリビールを選択してください。インタラクティブなエンゲージメントとユーザーの制御にはユーザートリガーリビールを選択してください。3つの戦略すべてがセキュアマッピングを通じて同じ基本的なセキュリティ利点を提供します。
  - q: 検証ハッシュはマッピングを明かさずにどのように公平性を証明しますか？
    a: ローンチ前の検証ハッシュは各メタデータファイルと完全なマッピングのSHA-256ハッシュです。ミント前にこれらのハッシュを公開することで、マッピングが事前に決定されていたことを証明します。リビール後、ユーザーは受け取ったメタデータをハッシュ化し、公開されたハッシュと比較して、割り当てが操作されていないことを検証できます。
---

## Summary

Core Candy Machineのボット対策は、プレースホルダーメタデータ、[Third-Party Signerガード](/ja/smart-contracts/core-candy-machine/guards/third-party-signer)によるバックエンド制御ミント、およびミント後のリビールプロセスを組み合わせて、希少性の狙い撃ちを防止し、Solana上でのNFTの公平な配布を確保します。 {% .lead %}

- プレースホルダーメタデータはリビールフェーズまで実際のトレイトデータをオンチェーン検査から隠します
- Third-Party Signerガードを持つバックエンドミントエンドポイントは、ボットが独自のミントトランザクションを生成することを防ぎます
- 事前生成された暗号マッピングがミントインデックスを最終メタデータにリンクし、リビール後に公開検証できます
- [Bot Taxガード](/ja/smart-contracts/core-candy-machine/guards/bot-tax)は失敗したミント試行にペナルティを課し、自動化されたスパムを抑止します

## ボット対策が重要な理由

ボット対策は、保護されていない[Core Candy Machine](/ja/smart-contracts/core-candy-machine)のローンチがメタデータ、ミント順序、トランザクション生成を自動化された悪用にさらすために不可欠です。適切な保護がないと、ボットは以下のことができます：
...
初期ミントフェーズ中にプレースホルダーメタデータを使用するよう[Core Candy Machine](/ja/smart-contracts/core-candy-machine)を設定し、潜在的に事前ランダム化された順序で：
...
実際のアセット更新実装については、UMIを使用してアセットメタデータ、名前、URIを更新する方法を説明する[Core Asset Updateドキュメント](/ja/smart-contracts/core/update)を参照してください。

リビールプロセスは、セキュアマッピングを使用してどの最終メタデータURIが各ミントされたアセットに対応するかを決定し、その後Coreのアップデート機能を使用してアセットを更新します。
...
{% callout type="warning" %}
**注意**：単一のセキュリティ対策が万全というものはありません。これらすべての戦略の組み合わせが、コミュニティにとって公平な体験を維持しながら、自動化された悪用に対する堅牢な防御を作成します。
{% /callout %}
...
4. **Candy Machine設定**：config linesでプレースホルダーメタデータを使用してデプロイ
5. **バックエンド保護**：必須のThird-Party SignerとBot Taxガードによりバックエンド経由ですべてのミントを制御
6. **リビールプロセス**：セキュアマッピングを使用してすべてのアセットをプレースホルダーから実際のメタデータに更新
7. **コミュニティ検証**：完全なマッピングを公開し、ユーザーがアセットを検証するためのツールを提供

## Notes

- すべてのCandy Machineアカウントデータはオンチェーンで公開読み取り可能です。希少性の狙い撃ちを防止したい場合、実際のメタデータURIをconfig linesに直接ロードしないでください。
- Third-Party Signerの秘密鍵はバックエンドサーバーにのみ保存し、クライアントサイドのコードに公開してはなりません。
- セキュアマッピングと検証ハッシュはミントローンチ前に生成してください。これらの値は遡って変更できません。
- メインネットにデプロイする前に、devnetでフロー全体（プレースホルダーミント、マッピング、リビール、検証）をテストしてください。
- レート制限だけでは不十分です。CAPTCHA、Third-Party Signer、Bot Taxガードと組み合わせて多層防御を構築してください。
- 決意のある攻撃者は継続的に弱点を探します。新しい攻撃ベクトルについて情報を得て、防御を更新してください。

## FAQ

### Candy Machineがガードを使用しているのに、なぜプレースホルダーメタデータが必要ですか？

ガードはミントプロセスを保護しますが、すべてのCandy Machineデータはオンチェーンで公開されています。プレースホルダーなしでは、ボットが実際のメタデータURIを事前取得し、希少性トレイトを分析し、ミント開始前に特定のインデックスをターゲットにできます。プレースホルダーメタデータはリビールまでトレイト情報を隠します。
...
[Third-Party Signerガード](/ja/smart-contracts/core-candy-machine/guards/third-party-signer)はバックエンドサーバーにのみ存在する秘密鍵からの暗号署名を要求します。攻撃者がフロントエンドをリバースエンジニアリングしても、秘密鍵へのアクセスなしにこの署名を偽造することはできません。これが、秘密鍵をクライアントサイドのコードに公開してはならない理由です。

### プロジェクトにどのリビール戦略を選択すべきですか？

即座のユーザー満足とサポート負担の軽減にはインスタントリビールを選択してください。コミュニティ全体の興奮とよりシンプルなミントインフラストラクチャにはイベントリビールを選択してください。インタラクティブなエンゲージメントとユーザーの制御にはユーザートリガーリビールを選択してください。3つの戦略すべてがセキュアマッピングを通じて同じ基本的なセキュリティ利点を提供します。

### 検証ハッシュはマッピングを明かさずにどのように公平性を証明しますか？

ローンチ前の検証ハッシュは各メタデータファイルと完全なマッピングのSHA-256ハッシュです。ミント前にこれらのハッシュを公開することで、マッピングが事前に決定されていたことを証明します。リビール後、ユーザーは受け取ったメタデータをハッシュ化し、公開されたハッシュと比較して、割り当てが操作されていないことを検証できます。

## Glossary

| Term | Definition |
|------|------------|
| Third-Party Signer | ミントトランザクションが有効と見なされる前に、指定されたバックエンドキーペアからの暗号署名を要求するCandy Machineガード。 |
| Bot Tax | ガード検証に失敗したミントトランザクションを持つウォレットに設定可能なSOLペナルティを課金し、自動化されたスパムを抑止するCandy Machineガード。 |
| Hidden Settings | すべてのミントされたアセットが同じ名前とURIを共有し、順次インデックス順にミントが進行するCandy Machine設定モード。 |
| Placeholder Metadata | ミント後のリビールまで実際のトレイトデータを隠すためにCandy Machineにロードされる一時的で非開示のメタデータ（例：「Mystery Asset」）。 |
| Merkle Proof | Merkleツリーから派生した暗号証明で、フルリストをオンチェーンに公開することなく、ウォレットの許可リストへの包含を検証します。 |
| Config Lines | 各アイテムの名前とメタデータURIを保存するCandy Machineのオンチェーンデータエントリ。アカウントをクエリする誰でも公開読み取り可能です。 |
| Reveal | 事前生成されたセキュアマッピングを使用して、各アセットのプレースホルダーメタデータを最終メタデータURIに更新するミント後のプロセス。 |
| Fisher-Yates Shuffle | 配列をランダムに並べ替えるための暗号的に偏りのないアルゴリズム。ここではセキュアなミント-メタデータマッピングの生成に使用されます。 |
