---
title: Third Party Signerガード
metaTitle: "Third Party Signerガード - Core Candy Machineガード | Metaplex"
description: "Third Party Signerガードは、Core Candy Machineでの各ミントトランザクションに事前定義されたアドレスの共同署名を要求し、集中型ミント認証とゲート付きアクセスワークフローを可能にします。"
keywords:
  - third party signer
  - Core Candy Machine
  - candy guard
...
**Third Party Signer**ガードは、Core Candy Machineでの各ミントトランザクションに事前定義されたアドレスの共同署名を要求し、集中型ミント認証とゲート付きアクセスワークフローを可能にします。 {% .lead %}
...
- ガード設定で設定された`signerKey`は、ミント設定で渡される署名者の公開鍵と正確に一致する必要があります。一致しない場合、トランザクションは失敗します。
- サードパーティ署名者のキーペアは、ミント時にトランザクションに署名するために利用可能でなければなりません。バックエンドゲート付きワークフローでは、署名者キーペアは通常サーバー側で保持され、ブロックチェーンに転送する前にトランザクションに署名します。
- このガードはミントを開始できる人を制限しません -- 指定された署名者が共同署名することのみを要求します。ウォレットレベルのアクセス制御には、[Allow List](/smart-contracts/core-candy-machine/guards/allow-list)などの他のガードと組み合わせてください。
