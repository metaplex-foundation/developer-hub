...
**アイテム挿入は[コンフィグライン設定](/ja/smart-contracts/candy-machine/settings#config-line-settings)を使用するキャンディマシンにのみ適用される**ことを覚えておくことが重要です。これは、[隠し設定](/ja/smart-contracts/candy-machine/settings#hidden-settings)を使用するキャンディマシンからミントされたNFTはすべて同じ「隠された」名前とURIを共有するためです。
...
さらに、[Sugar](/ja/smart-contracts/candy-machine/sugar)などのツールは、並列アップロード、プロセスのキャッシュ、失敗したアップロードの再試行により、JSONメタデータのアップロードを簡単にします。
...
[次のページ](/ja/smart-contracts/candy-machine/guards)では、これらすべてを可能にするキャンディガードについて話します。
