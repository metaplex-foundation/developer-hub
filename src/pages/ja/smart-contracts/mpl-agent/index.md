...
- **Metaplex Agent API** — [`mintAndSubmitAgent`](/agents/create-agent)が1回のAPI呼び出しでCoreアセットの作成とID登録を行います（推奨）
...
{% callout title="パスを選択" %}
- **最速の方法？** [Mint Agent API](/agents/create-agent)を使用 — 1回の呼び出しでエージェントを作成・登録
- **手動登録？** SDKのセットアップと個別のインストラクションは[はじめに](/smart-contracts/mpl-agent/getting-started)をご覧ください
