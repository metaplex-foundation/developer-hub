...
- `--offchain <path>`: 新しいメタデータを含むJSONファイルへのパス
...
- 少なくとも1つの更新フラグを提供する必要があります：`--name`、`--uri`、`--image`、`--offchain`、または`--edit`
- `--name`と`--uri`フラグは、`--offchain`または`--edit`と一緒に使用することはできません
- `--offchain`を使用する場合、メタデータファイルには有効な`name`フィールドが含まれている必要があります
