...
faqs:
  ...
  - q: agent をトークンローンチにリンクできますか？
    a: はい。--agentAsset に agent の Core asset アドレスを渡します。これにより agent の PDA からクリエイターフィーウォレットが自動導出されます。--agentSetToken を追加すると、トークンが agent に永久的にリンクされます（不可逆）。
---
...
- オプションで `--agentAsset` を使用してローンチを [agent](/ja/agents/mint-agent) にリンク
...
- **Agent サポート**: `--agentAsset` と任意の `--agentSetToken` でローンチを登録済み agent にリンク
...
| `--agentAsset <string>` | agent の Core asset アドレス — agent PDA からクリエイターフィーウォレットを自動導出 | いいえ | — |
| `--agentSetToken` | ローンチされたトークンを agent に永久的にリンク（**不可逆**）。`--agentAsset` が必要 | いいえ | `false` |
...
はい。`--agentAsset` に agent の Core asset アドレスを渡します。これにより agent の PDA からクリエイターフィーウォレットが自動導出されます。`--agentSetToken` を追加すると、トークンが agent に永久的にリンクされます（不可逆）。launchpool と bonding curve の両方で動作します。
