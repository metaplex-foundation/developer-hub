...
Genesis は Solana トークンローンチパッドであり、公正なオンチェーントークンローンチメカニズムを提供します。従来の ICO、IDO、中央集権的なトークンセールプラットフォームとは異なり、Launch Pool では市場がトークン配布を決定します。すべての参加者が預入額に比例してトークンを受け取り、公平で透明なトークン生成イベント（TGE）を実現します。SPL トークンの作成、資金調達、配布はすべてオンチェーンで行われ、仲介者は不要です。
